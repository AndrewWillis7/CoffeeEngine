...
public:
    ...
    // Borderless monitor-filling on Windows, EWMH _NET_WM_STATE_FULLSCREEN on
    // Linux (a silent no-op without an EWMH-compliant WM). Either path resizes
    // the real window, firing the Resize -> SetViewportSize callback main.cpp
    // already wires, so the camera picks up the new size with no extra plumbing.
